 ...
 * @returns {boolean}
 */
export const isObject = (obj: unknown): obj is ObjectType => !!obj && typeof obj === 'object' && obj.constructor === Object;
 ...
 */
export function mergeDeep<T extends ObjectType, S extends ObjectType>(target: T, source: S, isMergingArrays = false): T | S {
  ...
  Object.keys(source).forEach((key) => {
    ...
    if (Array.isArray(targetValue) && Array.isArray(sourceValue))
      if (isMergingArrays) {
        t[key] = targetValue.map((x, i) => (sourceValue.length <= i ? x : mergeDeep(x, sourceValue[i], isMergingArrays)));
        if (sourceValue.length > targetValue.length) t[key] = t[key].concat(sourceValue.slice(targetValue.length));
      } else {
